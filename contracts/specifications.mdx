...
# Online scenario example - uses preconditions to qualify the scenario
online_refund_contract = Contract(
    name="Online Refund Processing",
    requirements=[
        Precondition("Customer asks for refund"),
        Pathcondition("Verify order details"),
        Pathcondition("Document damage description"),
        Postcondition("Process refund or escalate appropriately")
    ]
)
