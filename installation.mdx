...
<Steps>
  ...
  <Step title="Option A: Set up Agent Contracts for Offline Verification">
    ...
    In the `agent-contracts` directory, start the agent contract service for verification.
  ...
  <Step title="Option B: Set up Agent Contracts for Runtime Certification">
    ...
    In the `agent-contracts` directory, start the agent contract service for runtime certification. You will need to set up a `specs.json` that specifies the runtime scenarios before starting the service (more details in [Scenarios and Specifications](/contracts/specifications)).
